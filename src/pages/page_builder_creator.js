...
function PageBuilderCreator(){
    ...
    function handleDelete(event){
        ...
        const commands = pageBuilder.commands.filter((cmd)=> cmd != event.target.id )
        setPageBuilder({
    ...
    return(
        ...
        <Main backgroundColor = 'colorThird' borderColor = 'colorFirst'>
            <Box backgroundColor='colorBlackFourth' shadowColor = 'colorSecond' borderRadius>
                ...
                <Wrapper>
                <FieldWrapper>
                {pageBuilder && pageBuilder.commands.map((cmd) => <Field key={cmd} value={cmd} readOnly name={cmd} />)}
                </FieldWrapper>
                <ButtonWrapper>
                {pageBuilder && pageBuilder.commands.map((cmd) => <Button small backgroundColor='colorFirst' blur shadowColor='colorBlackThird' borderRadius color='colorBlackFirst' borderColor='colorBlackFirst' key={cmd} id={cmd} onClick={handleDelete}>delete</Button>)}
                </ButtonWrapper>
                </Wrapper>
                <Button small backgroundColor='colorSecond' borderRadius onClick = {handleNext}>Next</Button>
            </Box>
        </Main>
        </> 
    )
